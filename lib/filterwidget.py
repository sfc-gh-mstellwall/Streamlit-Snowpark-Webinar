...
from snowflake.snowpark.functions import col, max, round, cast
...
class MyFilter:
    ...
    def __post_init__(self):
        ...
        if self.widget_type is st.select_slider:
            ...
            self._max_value = (
                ...
                .select(cast(round(max(col(self.table_column)), 0), "INTEGER"))
                .collect()[0][0]
            )
            ...
            self._df_method = "__eq__"
            self._options = self.session.table(MY_TABLE).select(col(self.table_column)).distinct().to_pandas()
    ...
    def get_filter_value(self):
        ...
        if self.widget_type in (st.checkbox, st.selectbox):
            # For .eq
    ...
    def create_widget(self):
        if self.widget_type is st.select_slider:
            ...
            base_label = "Choose"
        widget_kwargs = dict(label=f"{base_label} {self.human_name}", key=self.widget_id)
        if self.widget_type is st.select_slider:
